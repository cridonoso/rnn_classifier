...
def run(opt):
    ...
    pred_labels = tf.argmax(y_pred, 2)
    ...
    for pred_step in tf.transpose(pred_labels):
        precision, \
        recall, \
        f1, _ = precision_recall_fscore_support(y_true,
                                                pred_step,
                                                average='macro')
        f1_streaming.append(f1)
        rec_streaming.append(recall)
        prec_streaming.append(precision)


    h5f = h5py.File(os.path.join(opt.p, 'test', 'streaming.h5'), 'w')
    h5f.create_dataset('f1', data=f1_streaming)
    h5f.create_dataset('recall', data=rec_streaming)
    h5f.create_dataset('precision', data=prec_streaming)
